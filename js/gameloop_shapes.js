...
export const generateCylinder = (
  ...
  let differentialHeight = height / numParallelCircles;
  let circleDifferential = TAU / circlesResolution;

  let heightAccumulator = 0;

  for (
    let circlesIterator = 0;
    circlesIterator < numParallelCircles;
    ++circlesIterator
  ) {
    // generate circles
    let newCircleVertices = [];
    let newEdges = [];
    for (
      let angleIterator = 0;
      angleIterator < TAU;
      angleIterator += circleDifferential
    ) {
      let vertex = [
        Math.cos(angleIterator) * innerRadius,
        heightAccumulator - height / 2,
        Math.sin(angleIterator) * innerRadius,
      ];

      let edge = [
        solidData.numVertices + newCircleVertices.length,
        solidData.numVertices +
          ((newCircleVertices.length + 1) % circlesResolution),
      ];
      newEdges.push(edge);

      if (circlesIterator > 0) {
        edge = [
          solidData.numVertices - circlesResolution + newCircleVertices.length,
          solidData.numVertices + newCircleVertices.length,
        ];
        newEdges.push(edge);
      }
      newCircleVertices.push(vertex);
    }
    solidData.vertices.push(...newCircleVertices);
    solidData.edges.push(...newEdges);
    solidData.numVertices += newCircleVertices.length;
    solidData.numEdges += newEdges.length;

    heightAccumulator += differentialHeight;
  }
